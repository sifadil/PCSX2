...
void UpdateRegs(void)
{
	...
	wsprintf(text1,"0x%08X\0",iopRegs.GPR[32]);
	SendMessage(IOPGPRHIHandle,WM_SETTEXT,0,(LPARAM)(LPCTSTR)text1);
	wsprintf(text1,"0x%08X\0",iopRegs.GPR[33]);
...
void IOPDumpRegs(FILE * fp)
{
	...
	fprintf(fp,text1);
	fprintf(fp,"\n");
	sprintf(text1,"%x",iopRegs.GPR[32]);
	fprintf(fp,"GPR Register HI: ");
	fprintf(fp,text1);
	fprintf(fp,"\n");
	sprintf(text1,"%x",iopRegs.GPR[33]);
	fprintf(fp,"GPR Register LO: ");
	fprintf(fp,text1);
	fprintf(fp,"\n");
}
