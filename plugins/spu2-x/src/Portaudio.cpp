...
void Portaudio::Configure(uptr parent)
{		
	...
	INT_PTR ret;
	ret=DialogBoxParam(hInstance,MAKEINTRESOURCE(IDD_PORTAUDIO),(HWND)parent,_ConfigProc,1);
	if(ret==-1)
	{
...
static BOOL CALLBACK _ConfigProc(HWND hWnd,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
	...
	switch(uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			if(idx > 0)
			{
				...
				for(int j=0;j<Pa_GetDeviceCount();j++)
				{
					...
					if(info->hostApi == api_idx && info->maxOutputChannels > 0)
					{
						...
						if(wxString::FromAscii(info->name) == PA.GetDevice())
						{
							_idx = i;
						}
						i++;
			...
			SendMessage(GetDlgItem(hWnd,IDC_LATENCY),TBM_SETPOS,TRUE,PA.GetSuggestedLatencyMS());
			swprintf_s(temp, L"%d ms", PA.GetSuggestedLatencyMS());
			SetWindowText(GetDlgItem(hWnd,IDC_LATENCY_LABEL),temp);

			if(PA.GetSuggestedLatencyMinimal())
				SET_CHECK( IDC_MINIMIZE, true );
			else
				SET_CHECK( IDC_MANUAL, true );
				
			SET_CHECK( IDC_EXCLUSIVE, PA.GetWasapiExclusiveMode() );
		}
		break;
		...
		case WM_COMMAND:
		{
			...
			switch (wmId)
			{
				case IDOK:
				{
					...
					PA.SetApiId (SendMessage(GetDlgItem(hWnd,IDC_PA_HOSTAPI),CB_GETITEMDATA,idx,0));
					...
					if(info)
						PA.SetDevice (wxString::FromAscii( info->name ));
					else
						PA.SetDevice (L"default");
														
					int m_SuggestedLatencyMS = (int)SendMessage( GetDlgItem( hWnd, IDC_LATENCY ), TBM_GETPOS, 0, 0 );

					if( m_SuggestedLatencyMS < 10 ) m_SuggestedLatencyMS = 10;
					if( m_SuggestedLatencyMS > 200 ) m_SuggestedLatencyMS = 200;

					PA.SetSuggestedLatencyMS (m_SuggestedLatencyMS);

					PA.SetSuggestedLatencyMinimal (SendMessage(GetDlgItem(hWnd,IDC_MINIMIZE),BM_GETCHECK,0,0)==BST_CHECKED);
						
					PA.SetWasapiExclusiveMode (SendMessage(GetDlgItem(hWnd,IDC_EXCLUSIVE),BM_GETCHECK,0,0)==BST_CHECKED);
