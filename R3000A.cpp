...
{
	iopSetNextBranch( psxRegs.cycle, delta );
}
...
static __forceinline void IopTestEvent( IopEventId n, void (*callback)() )
{
	...
	if( iopTestCycle( psxRegs.sCycle[n], psxRegs.eCycle[n] ) )
	{
		psxRegs.interrupt &= ~(1 << n);
		callback();
	}
	else
		iopSetNextBranch( psxRegs.sCycle[n], psxRegs.eCycle[n] );
}
