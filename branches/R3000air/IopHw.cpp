...
//			return HW_DMA_PCR; // dma rest channel
		...
		case 0x1f801450:
			hard = psxHu32(add);
			PSXHW_LOG("%08X ICFG 32bit read %x", iopRegs.pc, hard);
			return hard;
...
//		case 0x1f801050: serial_write8(value); break;//serial port
		...
		case 0x1f801450:
			if (value) { PSXHW_LOG("%08X ICFG 8bit write %lx", iopRegs.pc, value); }
			psxHu8(0x1450) = value;
		return;
...
//			if (Config.SpuIrq) psxHu16(0x1070) |= 0x200;
		...
		case 0x1f801450:
			if (value) { PSXHW_LOG("%08X ICFG 16bit write %lx", iopRegs.pc, value); }
